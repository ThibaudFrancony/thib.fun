-- tibo.fun — anonymous guest access
-- Guests use Supabase anonymous Auth sessions. They can play while the
-- browser session exists, but do not receive persistent history or stats.

create or replace function private.guest_pseudo()
returns text
language sql
volatile
set search_path = ''
as $$
  select 'Joueur-' || upper(encode(extensions.gen_random_bytes(5), 'hex'));
$$;
    ...
    loop
      ...
      exception
        when unique_violation then
          -- A concurrent request may have provisioned this Auth user.
...
revoke all on function private.guest_pseudo() from public, anon, authenticated;
grant execute on function private.guest_pseudo() to service_role;
  ...
  select jsonb_build_object(
    ...
    'players', (
      select jsonb_agg(jsonb_build_object(
        ...
        'isGuest', exists (
          select 1 from auth.users u
          where u.id = mp.user_id and coalesce(u.is_anonymous, false)
        ),
...
as $$
  ...
  where exists (
    ...
    and not exists (
      select 1 from auth.users u
      where u.id = p_actor and coalesce(u.is_anonymous, false)
    )
    and exists (
      select 1 from private.site_members sm
...
create or replace function private.suppress_guest_history_entry()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if exists (
    select 1 from auth.users u
    where u.id = new.viewer_id and coalesce(u.is_anonymous, false)
  ) then
    return null;
  end if;
  return new;
...
drop trigger if exists suppress_guest_history_entry on public.history_entries;
create trigger suppress_guest_history_entry
before insert on public.history_entries
for each row execute function private.suppress_guest_history_entry();

create or replace function private.suppress_guest_player_stats()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if exists (
    select 1 from auth.users u
    where u.id = new.user_id and coalesce(u.is_anonymous, false)
  ) then
    return null;
  end if;
  return new;
...
drop trigger if exists suppress_guest_player_stats on public.player_game_stats;
create trigger suppress_guest_player_stats
before insert or update on public.player_game_stats
for each row execute function private.suppress_guest_player_stats();

create or replace function private.suppress_guest_pair_stats()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if exists (
    select 1 from auth.users u
    where (u.id = new.player_low or u.id = new.player_high)
...
drop trigger if exists suppress_guest_pair_stats on private.pair_game_stats;
create trigger suppress_guest_pair_stats
before insert or update on private.pair_game_stats
for each row execute function private.suppress_guest_pair_stats();
...
revoke all on function private.suppress_guest_history_entry() from public, anon, authenticated;
revoke all on function private.suppress_guest_player_stats() from public, anon, authenticated;
revoke all on function private.suppress_guest_pair_stats() from public, anon, authenticated;
