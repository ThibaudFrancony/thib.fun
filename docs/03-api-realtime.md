...
Le chat utilise un second canal privé `chat:<userId>`, abonné dès le chargement du résumé authentifié. Événements `chat.updated` (payload `{id: conversationId, version: seq}`) et `friend.updated` (`{id: friendshipId, version: 1}`). Ces messages ne transportent aucun contenu : le client relit toujours `/chat/summary` et la page de messages. Avant `subscribe`, `auth.getUser()` vérifie la session puis `realtime.setAuth()` transmet le JWT. Le premier abonnement réussi, les reconnexions et le retour au premier plan déclenchent une relecture ; un échec d'abonnement est retenté après 3 s. Une invalidation survenue pendant un GET impose une nouvelle lecture après sa réponse. Secours : conversation visible relue toutes les 5 s, résumé toutes les 30 s, présence toutes les 60 s. La politique SQL conserve le topic personnel exact, l'extension Broadcast et l'admission active, mais ne filtre pas la colonne `private` : la sonde d'autorisation Supabase a `private=false` même pour un canal privé. Les émetteurs et clients gardent `private: true`.
...
Realtime peut manquer, répéter ou retarder un message ; aucune règle ne dépend d'un message unique. Après réponse POST, afficher directement la vue personnelle du reçu de commit sans attendre Broadcast ni la relecture GET. Ce GET reste nécessaire pour les nouvelles métadonnées de phase et d'échéance ; lui seul devient le snapshot canonique. Afficher les actions localement avant réponse serveur quand leurs conséquences visibles sont déterministes, puis réconcilier avec le snapshot ou revenir en arrière sur refus. Ne jamais inventer score, carte piochée ou information cachée. BombParty peut vérifier un mot avec le lexique préchargé et afficher un passage de main provisoire ; l'échéance et la syllabe suivante restent serveur.
