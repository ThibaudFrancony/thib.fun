...
GitHub privé ; sans CI automatique (ni sur `push`, ni sur pull request). Depuis le 25/09/2026, chaque modification terminée et indépendante donne lieu à son propre commit, push vers `main` et vérification du push, sans attendre la fin de la tâche (3 demandes distinctes = 3 pushs), pour permettre un retour en arrière par commit. Chaque push embarque tout son périmètre, fichiers `supabase/migrations/*.sql` inclus. Vercel branch previews sur Supabase dev/preview isolé, données fictives. Projet production séparé avec corpus publié. Configurer Auth Site URL/redirect allowlist par environnement, pas wildcard tous domaines. Fonctions en région proche de la DB européenne ; mesurer latence depuis France avant ready BombParty.
...
CI automatique GitHub supprimée le 19/09/2026 (ancien `ci.yml` : install frozen-lockfile, lint/typecheck, unitaires, validation corpus, build, `test:matrix`, `docs:check`). Les mêmes contrôles restent obligatoires en local avant push (voir « Vérification pré-push » ci-dessous). Pas d'appel payant DeepSeek dans les vérifications standard, provider mock avec timeout/JSON invalide ; benchmark réel manuel daté avant activation et après changement modèle/prompt.
...
Vérification pré-push par défaut (sans Docker, avant chaque push) : `pnpm test:matrix`, `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm content:validate`, `pnpm docs:check`, `pnpm exec next build --webpack`. L'absence de recette Docker est notée en limitation dans `progression.md`, pas bloquante pour commit/push.
