...
Les RLS ci-dessus restent la barrière du Data API : un client ne lit jamais `history_entries` d'un autre joueur. Depuis la décision du 19/09/2026, le **serveur** expose volontairement l'historique complet d'un joueur sur `/profil/[id]` via le client `service_role` (même mécanisme admin que la lecture de son propre historique), après vérification de l'admission et refus des invités. Depuis le 19/09/2026, il n'existe plus de page ni d'API de détail de partie : l'historique n'est servi qu'en liste prévisualisée, sans lecture unitaire par `match_id`.
...
Pour Realtime : canal `user:<auth.uid()>`, privé. Politique SELECT `user_broadcast_receive` sur `realtime.messages` limitée à ce topic, extension broadcast et membre actif ; elle ne filtre pas la colonne `private` (la sonde d'autorisation Supabase vaut `false` même pour un canal privé — voir correctif du 19/09/2026 ci-dessous). Pas de droit d'envoyer des événements métier aux clients. Presence peut être ajouté au topic `presence:room:<roomId>` avec vérification d'appartenance par helper restreint ; **V1 utilise les heartbeats persistants et projections pour éviter ce second canal**. Ne pas confondre signal présence avec droit de jouer.

Chat et amis (18/09/2026) : un second topic privé `chat:<auth.uid()>` est autorisé par la politique `chat_broadcast_receive` pour les membres actifs ; il ne porte que les invalidations `chat.updated` et `friend.updated`. Aucune table de chat n'est publiée en Postgres Changes. La migration corrective `20260919101040_fix_chat_broadcast_authorization.sql` retire le filtre de colonne `private=true` de cette politique : il rejetait la sonde d'autorisation Supabase (valeur par défaut `false`). Le même correctif est appliqué au canal utilisateur le 19/09/2026 (`fix_user_broadcast_authorization`) : sans lui, tout abonnement `user:<uid>` est refusé en masse (`Unauthorized ... Channel topic: user:<uid>`), même avec un JWT valide. L'identité, le topic personnel exact, l'extension Broadcast et l'admission active restent obligatoires ; abonnements et émissions restent privés.
