...
Site privé de jeux entre amis, français, parties principalement à deux. Interface moderne et ludique, lisible et rapide à comprendre. **Décision utilisateur du 21 septembre 2026 : UNO et Géographie sont les références visuelles à préserver.** Harmoniser les sept autres jeux avec leur esthétique. La précision du 22 septembre autorise l’allègement et l’adaptation au viewport des neuf interfaces, en conservant la DA de UNO/Géographie. Cette demande remplace la palette crème/verte et la validation préalable d'un écran autrefois proposées comme défauts provisoires.
...
Plateaux : Trou Noir garde ses réserves et met la question en avant avec un halo ; TTMC propose dix tuiles de niveau avec sélection explicite ; Skyjo reçoit des cartes à dos violet texturé et des faces colorées ; BombParty met la syllabe au centre ; Compatibilité présente de grandes cartes de choix ; Longueur d'onde reçoit un cadran sombre gradué ; Bataille navale conserve son placement, ses états de grille et ses animations avec une enveloppe violette commune. L'entraînement BombParty adopte aussi ces surfaces. Révélations, résultats, champs et états d'erreur suivent la même palette. Les palettes et assets UNO/Géographie sont conservés ; `src/app/match-screen.css` adapte la disposition des neuf parties à la fenêtre.
...
Header : identité tibo.fun, navigation Jeux / Leaderboard / Profil, avatar et déconnexion. L'entrée « Historique » de l'ancien header est supprimée : l'historique se consulte désormais dans le profil. La route `/historique` redirige vers `/profil`. En fenêtre étroite (≤ 720 px, téléphone ou fenêtre réduite), les navigations du header sont masquées : seul le logo reste en haut à gauche avec un bouton chat à droite (pastille de non-lus) ; une barre d'onglets basse 100 % icônes (Jeux, Salon, Leaderboard, Profil) prend le relais et la déconnexion déménage sur la page profil. Desktop : largeur max 1200 px, 3 cartes prioritaires dans un bloc « À jouer maintenant », puis grille des 6 autres. Mobile : une colonne, cartes compactes ; aucun carrousel obligatoire cachant les priorités.
...
Header compact : retour salon, nom du jeu et progression utile, menu « Options de la partie » pour actualiser, abandonner (confirmation) et consulter les scores secondaires. Aucun bandeau de scores systématique, aucun paragraphe répétant une action évidente. Le plateau occupe la hauteur disponible, avec boutons courts et une seule indication de tour/attente/chrono. Réserves Trou Noir, vies BombParty et nombre de cartes adverses UNO restent visibles car ils influencent le coup. Les longues mains UNO sont paginées ; la carte piochée devient visible automatiquement. Les explications de quiz et récapitulatifs détaillés sont disponibles dans des dialogues, sans défilement du plateau.

Sélection locale modifiable avant validation, bouton désactivé pendant envoi, indicateur réseau si réponse tarde. Aucun brouillon privé diffusé (mot, placement ville, réponse quiz ou compatibilité). Pas besoin de chat : indice longueur-onde est l'unique texte envoyé volontairement à l'autre. Feedback interactif 150–300 ms (clic, validation, entrée de carte), séquence non bloquante et `prefers-reduced-motion` respecté. Les célébrations (podium, confettis, révélation de manche) peuvent durer 400–600 ms en mode ludique, toujours en `transform`/`opacity` et coupées en mouvement réduit. Son optionnel désactivé par défaut, activation explicite persistée localement ; pas nécessaire à la réussite.

## 7. Résultats et historique

Écran fin : victoire/défaite/égalité ou score partagé, valeurs finales et adversaire ; détails des manches à la demande, raison utile si forfait/interruption ; bouton « Rejouer » ramenant à la page du jeu pour une nouvelle partie. Le salon d'attente reste accessible par le lien « Salon » du header. Un résultat coopératif ne montre jamais « Tu as perdu ».
...
Recette minimale 360×640, 390×844, 844×390 (paysage), 768×1024 et 1440×900. Le plateau tient dans la fenêtre sans défilement horizontal ou vertical ; listes de recherche et dialogues secondaires peuvent défiler à la demande. En Bataille navale, le mode normal montre la grille entière ; « Grille agrandie » permet volontairement de déplacer la grille dans son panneau, sans faire défiler la page. Les cellules se dimensionnent selon le panneau, et les onglets remplacent les deux colonnes sur tablette comme sur téléphone. La hauteur suit le viewport visuel pour le clavier mobile ; le zoom conserve le déplacement natif d’accessibilité. Cibles tactiles >= 44×44 px. Focus visible, tabulation cohérente, boutons natifs, labels, erreurs reliées au champ. `aria-live=polite` pour changement de tour/résultat, pas annonce chaque tick. Cartes identifiées par texte/symbole en plus des couleurs. Dialogues piègent puis restaurent le focus.
...
- Une flèche discrète reste ancrée au bord droit de toutes les pages sur desktop. Elle ouvre une barre latérale violet sombre conforme à la maquette fournie : onglets « Général » et « Amis » avec pastilles de non-lus, compteur de messages, compteur approximatif de membres en ligne (activité < 2 min). En fenêtre étroite (≤ 720 px), la flèche est remplacée par un bouton rond en haut à droite du mini-header et le panneau devient un sheet sous le header (fermé à l'arrivée, la pastille signale les non-lus).
