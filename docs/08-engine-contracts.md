...
- UNO : après un DRAW où carte non jouable, finir le tour dans la même transition (pas after_draw inutile). Le contrôle wild4 se fait sur la main **avant le jeu** (l'annonce de dernière carte et sa pénalité ont été supprimées le 19/09/2026). Exception documentée (décision du 19/09/2026, fiche §5) : la projection expose `nextDrawCards` (fenêtre fixe de 8 cartes du sommet) et `nextDrawPlayable` au seul joueur actif pendant `playing` et pioche non vide, y compris pendant une pénalité pour animer la prise en bloc (`nextDrawCard` mono-carte conservé pour compatibilité) ; le reste de la pioche et la main adverse restent privés.
...
- Géographie : placements simultanés, ordre libre ; utiliser submitted séparé de placement null pour distinguer « pas joué » et « timeout ». Le premier PLACE_CITY garde phaseId et échéance partagée (pas de nouveau chrono), le second révèle. Timeout `turn_timeout` : tous les sièges sans placement reçoivent 0 puis reveal immédiat. `turnInRound` conservé mais ignoré ; actif placing = siège non validé. Challenge : SET_CITY_SELECTION ne réserve rien globalement ; CONFIRM réserve sous verrou, les drafts adverses ne sont pas consultables. La vue expose `avatarPreset` par joueur (photo via avatars du salon).
...
Géographie : match version0, phase P1, échéance partagée unique. A place en premier (ordre libre). API calcule sa distance privée, commit v1 garde P1 et l'échéance (pas de nouveau job), projection A voit son point, B voit seulement A a joué. B place : commit v2 crée P2 reveal, annule timeout, crédite deux scores et écrit round_result. Broadcast individuel -> GET des vues v2. A et B NEXT : première confirmation v3 garde P2, seconde v4 crée P3 nouvelle ville avec deadline complète ; job reveal P2 éventuellement reçu ensuite est cancelled. Au dernier round, finalisation écrit un seul résultat, deux historiques, deux stats et un agrégat duo.
