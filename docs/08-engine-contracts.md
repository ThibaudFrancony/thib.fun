...
- UNO : après un DRAW où carte non jouable, finir le tour dans la même transition (pas after_draw inutile). Le contrôle wild4 se fait sur la main **avant le jeu** (l'annonce de dernière carte et sa pénalité ont été supprimées le 19/09/2026). Exception documentée (décision du 19/09/2026, fiche §5) : la projection expose `nextDrawCard`/`nextDrawPlayable` au seul joueur actif pendant `playing`, hors pénalité et pioche vide, pour révéler la pioche sans attendre l'aller-retour ; le reste de la pioche et la main adverse restent privés.
