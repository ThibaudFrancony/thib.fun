...
| Profils/historique | 🟢 Code, 🟡 recette navigateur | `/profil` à deux colonnes (profil éditable + historique filtrable), `/profil/[id]` profil public lecture seule + historique complet en prévisualisation non cliquable, page liste `/historique` supprimée et redirigée, `/historique/duo/[id]` redirigé, détail `/historique/[id]` (page, API, composant) supprimé le 19/09/2026 ; fonctions `getPublicProfile`/`getPlayerGameStats` et route `/api/profiles/[id]/history` ; tests Vitest ajoutés ; recette distante à deux comptes restante |
...
| UNO | Implémenté | Moteur, projection, API/worker ; UI refondue en DA spatiale le 19/09 (assets `public/uno/`, composant `UnoCard`) ; cumul +2/+4 entre eux sans mélange depuis les règles `uno-2` (19/09, attente/riposte/prise, victoire immédiate sur dernière carte de pénalité) ; fluidité du 19/09 : vues optimistes à l'atterrissage des vols et `nextDrawCard`/`nextDrawPlayable` exposés au seul joueur actif (décision consignée) ; E2E fixtures 17/17 desktop et mobile |
