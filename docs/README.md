...
**Défauts de spécification exécutables choisis par l'agent :** exactement deux joueurs en V1, comptes sur invitation avec e-mail/mot de passe, règles et durées détaillées dans les fiches, noms d'affichage proposés, structure SQL, contrats API, gestion des égalités, échéances et infra de tâches. Ils évitent de laisser l'implémenteur inventer les détails. Les versions adaptées de Trou Noir/TTMC/cartes ne doivent pas être présentées comme des reproductions certifiées de règles commerciales.
