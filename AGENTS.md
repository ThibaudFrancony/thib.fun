...
- Sauf instruction explicite contraire dans la tâche active, pousser sur `main` dès qu'une modification est terminée et indépendante des autres, avec commit, push et vérification du push à chaque fois. Si la tâche demande 3 choses distinctes, livrer 3 commits/pushs séparés plutôt qu'un seul push de fin de tâche, pour permettre un retour en arrière par commit sur GitHub. Chaque push inclut l'ensemble de son périmètre : code, fichiers `supabase/migrations/*.sql`, tests, `progression.md` et documents réalignés. Ne jamais regrouper des modifications indépendantes dans un seul commit pour « gagner » un push. Une tâche de lecture ou de diagnostic sans fichier modifié ne committe ni ne pousse.
...
- Aucun secret dans `NEXT_PUBLIC_*`, le bundle, les projections, les logs ou les événements Realtime, sauf exception documentée (pioche UNO anticipée, décision du 19 septembre 2026). Le lexique BombParty est transmis séparément aux participants authentifiés par une route dédiée (décision du 26 septembre 2026). Aucune table d'état complet publiée dans Postgres Changes.
...
### Suppression du détail des parties (19 septembre 2026)
...
- Ancienne règle : la décision « Profils publics et historique complet » réservait le détail `/historique/[id]` (page, API, composant `history-detail.tsx`) aux participants, avec cartes cliquables (`Link`) et mention « Tu n'as pas joué cette partie » sinon ; `getProfileHistoryPage(viewerId, targetId)` marquait `viewerIsParticipant` via `viewerParticipates`, et `getHistoryDetail` servait score, raison, versions et manches.
- Nouvelle règle : l'historique dans `/profil` et `/profil/[id]` est une prévisualisation non cliquable uniquement (jeu, adversaire, issue, date, score). Aucune route `/historique/[id]`, aucune API `/api/history/[matchId]`, aucun composant de détail, aucun `Link` depuis les cartes. `getProfileHistoryPage(targetId)` ne distingue plus le visiteur ; `getHistoryEntry` est supprimé du repository.
- Périmètre : suppression de `src/app/historique/[matchId]/page.tsx`, `src/app/historique/history-detail.tsx`, `src/app/api/history/[matchId]/route.ts` ; `src/app/profil/profile-history.tsx` (cartes `div` statiques), `src/app/historique/_data.ts` (liste seule), `src/app/historique/history-helpers.ts` (plus de `viewerParticipates`/`historyReasonLabel`/`ProfileHistoryListItem`), `src/server/matches/repository.ts` (plus de `getHistoryEntry`), `src/app/historique/history-helpers.test.ts`, `src/app/globals.css` (plus de `.pf-detail*`/`.pf-history-locked`/hover lien), `src/app/profil/[id]/page.tsx` (sous-titre), `docs/02-database.md` §7, `docs/04-product-ui.md` §2/§4/§7/§13, `docs/07-implementation-status.md`, `docs/README.md`. Aucune migration, aucun changement de moteur, de transport Realtime ni de contrat de partie.
- Raison : la prévisualisation du profil suffit ; une page de détail par partie n'apporte rien et complique le parcours.

### Placements Géographie simultanés + avatars sur les guess (19 septembre 2026)

- Demande utilisateur : en Géographie, ne plus attendre que l'adversaire ait joué — placer et valider en même temps, le premier validé attend simplement la validation adverse avant révélation ; afficher la photo de profil avec le pseudo au-dessus des guess (pas un pointeur de couleur) pour voir directement qui est le plus proche.
- Ancienne règle : `docs/games/03-geographie.md` §1/§4 imposait deux placements successifs (`turnInRound` 0 puis 1, `actif = (firstSeat + round - 1 + turnInRound) % 2`, alternance du premier joueur, 60 s par tour) avec refus `NOT_YOUR_TURN` ; `docs/08-engine-contracts.md` §5/§6 décrivait ce flux ; la carte montrait des pointeurs de couleur anonymes (rond/carré) et la vue ne portait aucun avatar.
...
### Lexique BombParty chargé dans le navigateur (26 septembre 2026)

- Demande utilisateur : obtenir la même réponse instantanée qu'un jeu de syllabes en ligne, en chargeant le dictionnaire avant de jouer, puis valider le mot visuellement sans attendre le réseau.
- Ancienne règle : le lexique et l'index BombParty restaient exclusivement côté serveur ; la projection ne contenait ni mots possibles ni suggestions en compétition.
- Nouvelle règle : le lexique normalisé du pack de la partie peut être transmis aux deux participants authentifiés via une route dédiée et chargé en mémoire navigateur. Le joueur peut donc inspecter tous les mots acceptés par ce pack. Le navigateur vérifie localement forme, syllabe et répétition et anticipe visuellement le succès ; le serveur garde seul l'autorité sur l'échéance, l'ordre des tours, le tirage de la prochaine syllabe, le score et le résultat. Un refus ou un conflit rétablit la projection serveur. Pas de suggestions affichées par l'interface compétitive.
- Périmètre : distribution du lexique versionné aux participants, vérification locale, transition visuelle et réconciliation BombParty, documentation et tests. Aucun changement de moteur, de schéma ou de politique RLS.
- Raison : supprimer l'aller-retour ressenti pour la vérification d'un mot ; l'utilisateur accepte explicitement que le lexique soit consultable dans un site privé entre amis.

### Pioche UNO anticipée — exception à « aucun secret dans les projections » (19 septembre 2026)

- Demande utilisateur : le tour ne bascule qu'une seconde après la fin de l'animation, ce qui paraît lent ; révéler la carte piochée dès l'atterrissage du vol, en chargeant la prochaine carte de la pioche dans la projection, en acceptant le risque de triche puisque la partie est privée entre amis. Précision du même jour : connaître les **8 prochaines cartes** (fenêtre fixe) pour couvrir les prises de pénalité en bloc (+2/+4 cumulés), animer ces prises carte par carte, et supprimer les animations d'arrivée qui se rejouaient après les vols.
...
- Nouvelle règle : la projection UNO expose `nextDrawCards` (fenêtre fixe de 8 cartes du sommet de `drawPile`, `UNO_DRAW_PREVIEW_LIMIT`) et `nextDrawPlayable` (jouabilité de la première, calculée serveur) **au seul joueur dont c'est le tour**, pendant la phase `playing` et si la pioche n'est pas vide ; la fenêtre sert aussi pendant une pénalité en attente pour animer la prise. L'ancien champ `nextDrawCard` reste exposé aux clients déjà chargés (première carte, jamais pendant une pénalité). Le reste de la pioche, la main adverse et l'état complet restent privés ; aucune autre projection ni aucun autre jeu n'est concerné.
...
- Cas pratique explicite : le zoom naval « Grille agrandie » peut défiler dans son panneau pour garder des cases lisibles ; le plateau normal reste entièrement visible et la page ne s’agrandit pas. Les dialogues et recherches secondaires peuvent aussi défiler à la demande.
...
### Push incrémentaux par modification indépendante (25 septembre 2026)

- Demande utilisateur : pousser sur `main` dès qu'une modification est terminée et indépendante des autres ; si 3 choses distinctes sont demandées, faire logiquement un push par modification pour revenir facilement aux commits précédents sur GitHub ; inclure aussi les migrations Supabase dans ces pushs.
- Ancienne règle : sauf instruction contraire, toute tâche ayant produit une modification réelle se terminait par un unique commit puis un push vers `main` en fin de tâche.
- Nouvelle règle : chaque modification terminée et indépendante donne lieu à son propre commit, push vers `main` et vérification du push, sans attendre la fin de la tâche. Chaque push embarque tout son périmètre (code, `supabase/migrations/*.sql`, tests, `progression.md`, documents réalignés). Le push Git des fichiers de migration ne vaut jamais application distante : l'application d'une migration de production suit toujours la procédure connecteur + autorisation explicite. Ne pas forcer le push, ne pas réécrire l'historique, préserver les changements existants. La vérification pré-push sans Docker s'applique avant chaque push.
- Périmètre : `AGENTS.md` (§ Dépôt GitHub et règle ci-dessus), `progression.md` (entrée, points à savoir, journal), `docs/06-delivery-testing.md` §4/§5. Aucun changement moteur, projection, API, RLS, transport ni schéma.
- Raison : rendre chaque étape réversible séparément sur GitHub et éviter les gros commits multi-périmètres qui compliquent le retour en arrière.
...
À la fin d'une implémentation, donner : fonctionnalités réelles, tests effectués, limitations, configuration restante. Mettre à jour [la progression opérationnelle](progression.md) et, si nécessaire, [le suivi de spécification](docs/07-implementation-status.md), sans annoncer tous les jeux terminés lorsque seul un module l'est. Ne pas déployer ni appliquer de migration distante sans autorisation explicite de la tâche active ; chaque modification terminée et indépendante est committée et poussée vers `main` avec vérification, suivant la règle ci-dessus.
