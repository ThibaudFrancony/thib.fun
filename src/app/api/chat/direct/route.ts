import { openDirectConversation } from "@/server/chat/repository";
...
export async function POST(request: Request) {
  ...
  if (!member) return jsonError("ACCOUNT_REQUIRED", 403, "Crée un compte pour ouvrir une conversation privée.");
  const body = friendRequestInputSchema.safeParse(await request.json().catch(() => null));
  if (!body.success) return jsonError("INVALID_REQUEST", 400, "La demande de conversation est invalide.");
  try {
    const conversationId = await openDirectConversation(member.id, body.data.requestId, body.data.targetId);
