...
export async function GET(_request: Request, { params }: { params: Promise<{ matchId: string }> }) {
  ...
  try {
    ...
    if (snapshot.gameSlug !== "geographie" && snapshot.gameSlug !== "uno" && snapshot.gameSlug !== "trou-noir" && snapshot.gameSlug !== "ttmc") return jsonError("GAME_NOT_READY", 409, "Cette partie n'est pas disponible.");
