...
export function RoomLobby({ roomId, roomManagementEnabled }: { roomId: string; roomManagementEnabled: boolean }) {
  ...
  const onSnapshotApplied = useCallback((next: RoomView) => {
    if (next.currentMatchId) {
      router.push(`/parties/${next.currentMatchId}`);
      return;
    }
    if (!next.members.some((member) => member.id === next.viewerId)) {
      router.push(`/jeux/${next.gameSlug}`);
    }
  }, [router]);
    ...
    return (
      <div role="alert" className="mt-10 rounded-2xl bg-red-50 p-5 text-red-700">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Link href="/" className="rounded-full border border-red-200 px-4 py-2 font-bold">Retour aux jeux</Link>
        </div>
      </div>
    );
  }
  ...
  const roomDraft = draft?.roomVersion === room.version ? draft : null;
  ...
  const updateDraft = (changes: Partial<Omit<LobbyDraft, "roomVersion">>) => {
    ...
    setDraft({ ...current, ...changes, roomVersion: room.version });
  };
      ...
      <div className="geo-panel geo-lobby-panel">
        <div className="geo-lobby-heading">
          <div>
            ...
            <p className="geo-panel-note">Partage ce code à ton partenaire.</p>
            {expiration && <p className="mt-2 text-sm font-bold text-[var(--muted)]">{expiration}</p>}
          </div>
          <button type="button" onClick={() => void copyRoomCode()} className="geo-secondary-button geo-copy-button">Copier</button>
        </div>
        ...
        <section className="mt-5 grid gap-5 md:grid-cols-2">
          {isHost && room.status === "waiting" && (
            <div className="geo-panel p-5">
              ...
              <select id="room-game" value={selectedGameSlug} onChange={(event) => updateDraft({ selectedGameSlug: event.target.value })} disabled={busy} className="mt-2 min-h-11 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2 font-bold">
