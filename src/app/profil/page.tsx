...
export default async function ProfilePage() {
  ...
  const effective = account.member.effectiveName;
    ...
    <main className="min-h-screen">
      ...
      <div className="mx-auto max-w-3xl px-5 pb-16 pt-8 sm:px-8">
        ...
        <section className="mt-8 rounded-[1.5rem] border border-[var(--line)] bg-[var(--card)] p-6 shadow-[0_12px_30px_rgba(20,33,29,0.06)] sm:p-8">
          <p className="text-sm font-bold uppercase tracking-[0.16em] text-[var(--orange)]">Mon profil</p>
          <h1 className="mt-3 text-4xl font-black tracking-[-0.05em]">Bienvenue, {effective}.</h1>
          <dl className="mt-8 grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-2xl bg-[var(--paper-deep)] p-4"><dt className="text-xs font-black uppercase tracking-[0.12em] text-[var(--muted)]">Nom affiché</dt><dd className="mt-2 font-bold">{account.member.displayName ?? "— (nom de création affiché)"}</dd></div>
          </dl>
          <p className="mt-6 text-[var(--muted)]">Ta photo et ton nom affiché apparaissent dans les groupes et les parties.</p>
          <ProfileEditor accountName={account.member.accountName} displayName={account.member.displayName} avatarPreset={account.member.avatarPreset} avatarPath={account.member.avatarPath} />
