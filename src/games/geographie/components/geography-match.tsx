...
function ForfeitControl({ available, busy, claim, resign }: { available: boolean; busy: boolean; claim: () => void; resign: () => void }) {
  const phase = available ? "Le forfait est disponible : ton partenaire n'a plus donné de signe depuis au moins 90 secondes." : "Le forfait devient disponible après 90 secondes sans signal.";
  return <div className="geo-forfeit-panel"><p>Partie bloquée ?</p><span>{phase}</span><div className="flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={resign} className="geo-danger-button">Abandonner</button><button type="button" disabled={!available || busy} onClick={claim} className="geo-secondary-button">{available ? "Réclamer le forfait" : "Forfait indisponible"}</button></div></div>;
}
